    ...
    class Template {
        ...
        */
        ...
        static private function parseContent($content, $params) {
            ...
            //TABLE
            $content = self::arrayToTable($content, $params);
        ...
        static private function arrayToTable($content, $params) {
            if (preg_match_all('#<istable \${(\w+)} *\/>#U', $content, $matches)) {
                $property   = [];
                
                foreach ($matches[1] as $match) {
                    $tmp    = $match;
                    $table  = '<table id="tbl_'.$tmp.'" border="1" class="table table-bordered table-striped table-condensed sortable"><thead><tr>';
                    if (isset($params[$match])) {
                        $array      = $params[$match];
                        
                        // amélioration : table dans un template
                        
                        // We find the class and its attibutes
                        $myClass    = ucfirst(substr($match, 0, -1));
                        $myInstance = new $myClass();
                        $attributes = $myInstance->getAttributes();
                        
                        // We create the array containing the mysql table
                        foreach ($array as $values) {
                            foreach ($attributes as $attribute => $value) {
                                ...
                                if (method_exists($myClass, $method)) {
                                    ...
                                    $property[$attribute] = $values->$method();
                                }
                            }
                            $properties[] = $property;
                        }
                        ...
                        foreach ($properties[0] as $key => $col) {
                            $table .= '<th>'.$key.'</th>';
                        }
                        ...
                        foreach ($properties as $row) {
                            ...
                            $table .= '<td style="width: 200px;"><input type="button" class="btn-edit"  onclick="location.href=\''.ucfirst($tmp).'-Update-'.$currentId.'\';" value="Editer" />
                                    <input type="button" class="btn-remove" onclick="location.href=\''.ucfirst($tmp).'-Delete-'.$currentId.'\';" value="Supprimer" /></td></tr>';
                        }
                        $table .= '</tbody></table>';
                        $content = str_replace('<istable ${'.$tmp.'} />', $table, $content);
                    }
                }
            }
            return $content;
        ...
        static private function parseObjectVar($content, $params) {
            if (preg_match_all('#\${([a-zA-Z.]+)}#', $content, $matches)) {
                ...
                foreach ($matches[1] as $match) {
                    ...
                    if (!is_array($params[$attributes[0]])) {
